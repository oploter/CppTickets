...
**Применение при передаче аргументов**
* ``const type& a_ref = temporary_object`` - констатные ссылки используются, чтобы продлить время жизни временного объекта ([temporary object lasts only until the end of the full expression in which it appears](https://herbsutter.com/2008/01/01/gotw-88-a-candidate-for-the-most-important-const/) --> to the lifetime of the reference itself) 
...
* const квалификатор надо писать и в объявлении и в определении функции
* Пример:
